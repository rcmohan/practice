...
public class CryptSolution {
	...
	boolean solution(String[] crypt, char[][] solution) {
	    ...
	    int word1 = getval(crypt[0], map);
	    int word2 = getval(crypt[1], map);
	    int word3 = getval(crypt[2], map);
	    return word3 == word1 + word2;
	}

	int getval(String s, Map<Character,Character> map) {
	    if(s.length() == 1) {
	        return Integer.parseInt("" + map.get(s.charAt(0)));
	    }
	    int sum = 0;
	    int m = 1;
	    int j = s.length();
	    char lastchar = '1';
	    while(j --> 0) {
	        char c = map.get(s.charAt(j));
	        sum += Integer.parseInt("" + c) * m;
	        m = m * 10;
	        lastchar = c;
	    }
	    
	    return lastchar == '0' ? 100-sum : sum;
